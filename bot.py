...
async def bulk_add_groups(event):
    lines = event.raw_text.strip().splitlines()[1:]  # Skip .addgroup line
    if not lines:
    ...
    for line in lines:
        ...
        if not link.startswith("http"):
            failed.append((link, "Invalid link format"))
            continue
        try:
...
async def help_command(event):
    help_text = (
        "**🤖 Available Commands:**\n"
        "📦 `.addgroup` + links (one per line) — Save multiple groups\n"
        "🎯 `.forwardto <t.me/link>` — Forward messages only to a specific group\n"
        "🧹 `.clearforwardto` — Remove all `.forwardto` restrictions\n"
        "🕒 `.time <10m>` — Set post interval (min 10 min)\n"
        "⏱ `.delay <seconds>` — Delay between messages\n"
        "📁 `.folder <name>` — Target a folder's groups (optional)\n"
        "📊 `.status` — View current status and plan\n"
        "👤 `.info` — Account and Telegram details\n"
        "❌ `exit` — Detach terminal (bot stays running)"
    )
